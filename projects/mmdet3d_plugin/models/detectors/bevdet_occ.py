...
@DETECTORS.register_module()
class BEVDetOCCAX(BEVDetOCC):
    def __init__(self,
                 wocc=True,
                 wdet3d=True,
                 uni_train=True,
                 **kwargs):
        super(BEVDetOCCAX, self).__init__(**kwargs)
        self.wocc = wocc
        self.wdet3d = wdet3d
        self.uni_train = uni_train
    ...
    def forward_ori(
        ...
        # x = ax_bev_pool_v2(depth, tran_feat, ranks_depth, ranks_feat, ranks_bev,
        #                     n_points, bev_feat_shape)    # (B, Dz, Dy, Dx, C)
        x = AXBEVPoolv2.apply(depth.contiguous(), tran_feat.contiguous(),
                               ranks_depth, ranks_feat, ranks_bev,
                               n_points, bev_feat_shape)
