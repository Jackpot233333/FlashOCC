...
def main():
    ...
    assert model.__class__.__name__ in ['BEVDetOCCAX']
    for i, data in enumerate(data_loader):
        ...
        onnx_input = (img.float().contiguous(), metas[0].int().contiguous(),
            ...
            torch.tensor([metas[3]]).int().contiguous())
        dynamic_axes = None
        input_names = [
        ...
        with torch.no_grad():
            ...
            torch.onnx.export(
                model,
                onnx_input,
                "./outputs/" + model_prefix + '_with_argmax.onnx',
                opset_version=13,
                dynamic_axes=dynamic_axes,
                input_names=input_names,
                output_names=output_names)
            print('output_names:', output_names)
            print('====== onnx is saved at : ', "./outputs/" + model_prefix + '_with_argmax.onnx')
            # check onnx model
            onnx_model = onnx.load("./outputs/" + model_prefix + '_with_argmax.onnx')
            try:
                onnx.checker.check_model(onnx_model)
            except Exception:
                print('ONNX Model Incorrect')
